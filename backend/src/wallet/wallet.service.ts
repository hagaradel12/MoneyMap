...
export class WalletService {
  ...
  //adds wallet to user's wallet variable
  async create(email: string, walletDto: CreateWalletDto): Promise<Wallet> {
    const wallet = new this.walletModel(walletDto);
    const user = await this.userService.findUserByEmail(email);
    if (!user) {
      throw new Error('User not found');
    }
    user.wallet = wallet._id; //assign wallet._id to user's wallet variable
    await user.save(); //save users with updated wallet 
    return wallet.save();
  }
...
//and the amount of the expense
  async incBalance(email: string, amount: number) {
    //+ findWalletByUsername in user to find wallet of user by their username
    //it returns wallet populated
    const wallet: WalletDocument = await this.userService.getWalletByEmail(email);
    const new_balance = wallet.balance + amount;
    return this.walletModel.findByIdAndUpdate(wallet._id, {balance: new_balance}, { new: true });
  }

  async decBalance(email: string, amount: number) {
    //+ findWalletByUsername in user to find wallet of user by their username
    //it returns wallet populated
    const wallet = await this.userService.getWalletByEmail(email);
    const new_balance = wallet.balance - amount;
    return this.walletModel.findByIdAndUpdate(wallet._id, {balance: new_balance}, { new: true });
  }
...
async deleteWallet(id: mongoose.Types.ObjectId) {
  const wallet = await this.walletModel.findById(id);
  if (!wallet) throw new Error('Wallet not found');

  // Delete all associated expenses
  await this.expensesModel.deleteMany({ _id: { $in: wallet.expenses } });

  // Delete wallet itself
  return this.walletModel.findByIdAndDelete(id);
}

async clearExpenses(id: mongoose.Types.ObjectId) {
  const wallet = await this.walletModel.findById(id);
  if (!wallet) throw new Error('Wallet not found');
  ...
  // Clear expenses list in wallet
  return this.walletModel.findByIdAndUpdate(id, { expenses: [] }, { new: true });
}
async clearIncome(id: mongoose.Types.ObjectId) {
  const wallet = await this.walletModel.findById(id);
  if (!wallet) throw new Error('Wallet not found');

  return this.walletModel.findByIdAndUpdate(id, { balance: 0 }, { new: true });
}
