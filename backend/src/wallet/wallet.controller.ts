...
async deleteWallet(@Param('id') id: string) {
  const oid = new mongoose.Types.ObjectId(id);
  return this.walletService.deleteWallet(oid);
}

@Put('clear-expenses/:id')
clearExpenses(@Param('id') id: string) {
  const oid = new mongoose.Types.ObjectId(id);
  return this.walletService.clearExpenses(oid);
}

@Put('clear-income/:id')
clearIncome(@Param('id') id: string) {
  const oid = new mongoose.Types.ObjectId(id);
  return this.walletService.clearIncome(oid);
}
